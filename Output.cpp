...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	int y = (UI.CellHeight)*(cellPos.VCell()) + UI.ToolBarHeight;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_COIN_SET] = "images\\Menu_Coin.jpg";	
	MenuItemImages[ITM_ADD_CARD] = "images\\Menu_Card.jpg";	
	MenuItemImages[ITM_COPY_CARD_OR_COINS] = "images\\Menu_Copy.jpg";	
	MenuItemImages[ITM_CUT_CARD_OR_COINS] = "images\\Menu_Cut.jpg";	
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Menu_DiceInput.jpg";
	...
	MenuItemImages[ITM_SELL_CITY] = "images\\Menu_SellCity.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\Menu_NewGame.jpg";
	MenuItemImages[ITM_SWITCH_TO_DESIGN_MODE] = "images\\Menu_SwitchToGame.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	//To revise mario // revised

	int cells;
	cells=((fromCell.VCell()-toCell.VCell()));
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	int cx [4]={x12,x12-xChange,x12,x12+xChange};
	int cy [4]={y1-yChange,y1,y1+yChange,y1};
	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
	pWind->DrawPolygon(cx,cy,4);
...
void Output::DrawCoinSet(const CellPosition & cellPos) const
{
	...
	int cellStartX = GetCellStartX(cellPos);
	int cellStartY = GetCellStartY(cellPos);
	int w = UI.CellWidth, h = UI.CellHeight;
	string Image = "images\\Menu_Coin.jpg";


	pWind->DrawImage(Image, cellStartX, cellStartY, w, h);
